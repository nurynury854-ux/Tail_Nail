...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  // line_id and the branch name are needed to tell the customer their
  // appointment is off. Both stay server-side — neither is returned to the
  // browser, so this doesn't widen what the calendar can see.
  ...
  // Tell the customer — after the realtime signal and the audit log, so a slow
  // push never delays the calendar update or the record of who cancelled.
  ...
  if (booking.line_id) {
    try {
      ...
      // Start with the OA that actually reached this customer before, then walk
      // the rest: a push through an OA they haven't added fails with 400.
      const provenBranchId = await lookupOaBranch(admin, booking.id)
      const result = await sendCustomerPush(
        booking.line_id,
        message,
        buildCandidateBranchIds(provenBranchId, booking.branch_id),
      )
      lineNotificationSent = result.sent
      if (!result.sent) {
        console.warn(
          `Cancellation LINE message FAILED for booking ${booking.id} ` +
            `(branch ${booking.branch_id}). Tried: ${result.errors.join(' | ')}`,
        )
      }
    } catch (lineError) {
      // The appointment IS cancelled either way — never fail the request here.
      console.warn('Failed to send cancellation LINE message:', lineError)
    }
  } else {
    console.warn(`Cancellation not sent for booking ${booking.id}: no customer LINE userId.`)
  }

  // The UI reports this so 已取消預約 doesn't imply a notification that never went.
  return NextResponse.json({ success: true, line_notification_sent: lineNotificationSent })
}
